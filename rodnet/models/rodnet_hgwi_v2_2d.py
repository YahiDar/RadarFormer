...
from .backbones.hgwi_2d import RadarStackedHourglass
from .modules.mnet import MNet



class RODNetHGwIDCN_2d(nn.Module):
    def __init__(self, in_channels,win_size, n_class, stacked_num=1, mnet_cfg=None, dcn=True):
        super(RODNetHGwIDCN_2d, self).__init__()
        self.dcn = dcn
        if dcn:
...
if __name__ == '__main__':
    testModel = RODNetHGwIDCN_2d().cuda()
    x = torch.zeros((1, 2, 30, 128, 128)).cuda()
    testModel(x)
