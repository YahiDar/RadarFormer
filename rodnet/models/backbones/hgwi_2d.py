...
class RadarStackedHourglass(nn.Module):
    ...
    def __init__(self, in_channels, win_size, n_class, stacked_num=1, conv_op=None, use_mse_loss=False):
        ...
        # self.conv1c = nn.Conv3d(in_channels=80, out_channels=32,
        #                         kernel_size=(4, 5, 5), stride=(1, 1, 1), padding=(0, 2, 2))
        # self.conv1b = nn.Conv3d(in_channels=32, out_channels=16,
        #                         kernel_size=(4, 5, 5), stride=(1, 1, 1), padding=(0, 2, 2))
        # self.conv1a = nn.Conv3d(in_channels=16, out_channels=80,
        #                         kernel_size=(4, 5, 5), stride=(1, 1, 1), padding=(0, 2, 2))
        ...
        for i in range(stacked_num):
            self.hourglass.append(nn.ModuleList([RODEncode(), RODDecode(win_size=self.win_size),
...
class InceptionLayerConcat(nn.Module):
    ...
    def __init__(self, kernal_size, in_channel, stride):
        ...
        self.branch3b = nn.Conv2d(in_channels=32, out_channels=32,
                                 kernel_size=(kernal_size[0],kernal_size[1]),stride=stride,
                                 padding=(paddingX,paddingY))
        # self.branch1 = nn.Conv3d(in_channels=in_channel, out_channels=16,
        #                          kernel_size=(5, kernal_size[0], kernal_size[1]), stride=stride,
        #                          padding=(2, paddingX, paddingY))
        # self.branch2a = nn.Conv3d(in_channels=in_channel, out_channels=32,
        #                           kernel_size=(5, kernal_size[0], kernal_size[1]), stride=(1, 1, 1),
        #                           padding=(2, paddingX, paddingY))
        # self.branch2b = nn.Conv3d(in_channels=32, out_channels=32,
        #                           kernel_size=(9, kernal_size[0], kernal_size[1]), stride=stride,
        #                           padding=(4, paddingX, paddingY))
        # self.branch3a = nn.Conv3d(in_channels=in_channel, out_channels=32,
        #                           kernel_size=(5, kernal_size[0], kernal_size[1]), stride=(1, 1, 1),
        #                           padding=(2, paddingX, paddingY))
        # self.branch3b = nn.Conv3d(in_channels=32, out_channels=32,
...
class RODEncode(nn.Module):
    ...
    def __init__(self):
        ...
        self.inception1 = InceptionLayerConcat(kernal_size=(5, 5), in_channel=80, stride=(2, 2))
        self.inception2 = InceptionLayerConcat(kernal_size=(5, 5), in_channel=80, stride=(2, 2))
        self.inception3 = InceptionLayerConcat(kernal_size=(5, 5), in_channel=80, stride=(2, 2))

        self.skip_inception1 = InceptionLayerConcat(kernal_size=(5, 5), in_channel=80, stride=(2, 2))
        self.skip_inception2 = InceptionLayerConcat(kernal_size=(5, 5), in_channel=80, stride=(2, 2))
        self.skip_inception3 = InceptionLayerConcat(kernal_size=(5, 5), in_channel=80, stride=(2, 2))
        # self.conv4a = nn.Conv3d(in_channels=32, out_channels=32,
        #                         kernel_size=(9, 5, 5), stride=(1, 1, 1), padding=(4, 2, 2))
        # self.conv4b = nn.Conv3d(in_channels=32, out_channels=32,
        #                         kernel_size=(9, 5, 5), stride=(1, 2, 2), padding=(4, 2, 2))
        # self.conv5a = nn.Conv3d(in_channels=32, out_channels=32,
        #                         kernel_size=(9, 5, 5), stride=(1, 1, 1), padding=(4, 2, 2))
        # self.conv5b = nn.Conv3d(in_channels=32, out_channels=32,
        #                         kernel_size=(9, 5, 5), stride=(1, 2, 2), padding=(4, 2, 2))
        ...
        # self.bn4a = nn.BatchNorm3d(num_features=32)
        # self.bn4b = nn.BatchNorm3d(num_features=32)
        # self.bn5a = nn.BatchNorm3d(num_features=32)
        # self.bn5b = nn.BatchNorm3d(num_features=32)
        self.relu = nn.ReLU()
