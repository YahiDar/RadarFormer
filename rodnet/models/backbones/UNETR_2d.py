...
class RadarStackedHourglass(nn.Module):
    ...
    def __init__(self, in_channels, n_class, stacked_num = 1, conv_op=None, use_mse_loss=False,
                 ...
                 mlp_dim = 3072, num_layers = 12, num_heads = 12, win_size=16):
...
class DecompConv3d(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, window_size, kernel_size = 3,  stride = 1):
        super(DecompConv3d, self).__init__()
...
class InceptionLayerTrans(nn.Module):
    ...
    def __init__(self, kernal_size, in_channel, out_channel, window_size, stride = 1):
        ...
        self.branch2b = DecompConv3d(in_channels= out_channel, out_channels=out_channel,
        ...
        self.branch3b = DecompConv3d(in_channels= out_channel, out_channels=out_channel,
                                    kernel_size = kernal_size, window_size = window_size*2, stride = stride)

        self.merge = DecompConv3d(in_channels= out_channel*2, out_channels=out_channel,
...
class SingleConv(nn.Module):
    ...
    def __init__(self, kernal_size, in_channels, out_channels, window_size, stride = 1):
        ...
        self.block1 = DecompConv3d(in_channels = in_channels, out_channels = out_channels,
...
class PostConv(nn.Module):
    ...
    def __init__(self, kernal_size, in_channels, window_size, out_channels, stride =1):
        ...
        self.block1 = DecompConv3d(in_channels = in_channels, out_channels = b1channel,
                                window_size = window_size, kernel_size = kernal_size, stride = stride)
                                
        self.block2 = DecompConv3d(in_channels = b1channel, out_channels = out_channels,
                                window_size = window_size, kernel_size = kernal_size, stride = stride)
        #self.relu = nn.ReLU()
        self.bn1 = nn.BatchNorm2d(num_features = out_channels)
...
class ConvRes(nn.Module):
    ...
    def __init__(self, kernal_size, in_channels, out_channels, window_size, stride =1 ):
        ...
        if self.in_channels != self.out_channels:
            self.ch_conv = DecompConv3d(in_channels = in_channels, out_channels = out_channels,
            window_size = window_size, kernel_size = kernal_size, stride = stride)
            self.block1 = DecompConv3d(in_channels = out_channels, out_channels = out_channels//2,
            window_size = window_size, kernel_size = kernal_size, stride = stride)
        else:
            self.block1 = DecompConv3d(in_channels = in_channels, out_channels = out_channels//2,
                                window_size = window_size, kernel_size = kernal_size, stride = stride)
        self.block2 = DecompConv3d(in_channels = out_channels//2, out_channels = out_channels,
                                window_size = window_size, kernel_size = kernal_size, stride = stride)
...
class UNETR(nn.Module):
    ...
    def __init__(
        ...
        self.bn12 = nn.BatchNorm3d(in_channels)
        self.relu = nn.ReLU()

        self.res1_decoder1 = InceptionLayerTrans(kernal_size=3, in_channel = hidden_size,
                                        window_size = self.feat_size[0], out_channel = feature_size * 2, stride=1)
        self.res1_decoder2 = UnetrPrUpBlock(
            spatial_dims=2,
            in_channels=feature_size * 2,
            out_channels=feature_size * 2,
            num_layer=self.num_samples - 2,
            kernel_size= self.rf[0][0],
            stride=1,
        ...
        self.res1_decoder4 = SingleConv(kernal_size = self.rf[0][0], in_channels = in_channels,
                                        window_size = img_size[0], out_channels= feature_size*2, stride = 1)
        ...
        self.res2_decoder1 = InceptionLayerTrans(kernal_size=3, in_channel = hidden_size,
                                        window_size = self.feat_size[0], out_channel = feature_size * 4, stride=1)
        self.res2_decoder2 = UnetrPrUpBlock(
            spatial_dims=2,
            in_channels=feature_size * 4,
            out_channels=feature_size * 4,
            num_layer=self.num_samples - 2,
            kernel_size=self.rf[0][1],
            stride=1,
            upsample_kernel_size=2,
            norm_name=norm_name,
            conv_block=False,
        ...
        self.res3_decoder1 = InceptionLayerTrans(kernal_size=3, in_channel = hidden_size,
                                        window_size = self.feat_size[0], out_channel = feature_size * 8 if (feature_size * 8) <=64 else 64, stride=1)
        
        
        self.res3_decoder2 = UnetrPrUpBlock(
            spatial_dims=2,
            in_channels=feature_size * 8 if (feature_size * 8) <=64 else 64,
            out_channels=feature_size * 8 if (feature_size * 8) <=64 else 64,
            num_layer=self.num_samples - 2,
            kernel_size=self.rf[0][2],
            stride=1,
            upsample_kernel_size=2,
            norm_name=norm_name,
            conv_block=False,
        ...
        self.res4_decoder1 = InceptionLayerTrans(kernal_size=3, in_channel = hidden_size,
                                        window_size = self.feat_size[0], out_channel = feature_size * 8 if (feature_size * 8) <=64 else 64, stride=1)
        
        
        self.res4_decoder2 = UnetrPrUpBlock(
            spatial_dims=2,
            in_channels=feature_size * 8 if (feature_size * 8) <=64 else 64,
            out_channels=feature_size * 8 if (feature_size * 8) <=64 else 64,
            num_layer=self.num_samples - 2,
            kernel_size=self.rf[0][3],
            stride=1,
            upsample_kernel_size=2,
            norm_name=norm_name,
            conv_block=False,
        ...
        self.merge1 = nn.Conv3d(
              kernel_size = 3,
              in_channels = in_channels,
              out_channels = in_channels//2,
              padding = 1,
              stride = 1)
        self.merge2 = nn.Conv3d(
              kernel_size = 3,
              in_channels = in_channels//2,
              out_channels = feature_size,
              padding = 1,
        ...
        self.bn22 = nn.BatchNorm3d(feature_size)
        
        self.out = UnetOutBlock(spatial_dims=3, in_channels=feature_size, out_channels=self.out_channels)  # type: ignore
    ...
    def forward(self, x_in):
            x_in = self.relu(self.bn11(self.encoder11(x_in)))
            x_in = self.relu(self.bn12(self.encoder12(x_in)))
            ...
            #merge with original
            x4 += x_in
            x4 = self.relu(self.bn21(self.merge1(x4)))
            x4 = self.relu(self.bn22(self.merge2(x4)))
            out = self.out(x4)
            return out
