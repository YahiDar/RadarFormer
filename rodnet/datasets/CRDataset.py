...
class CRDataset(data.Dataset):
    ...
    def __init__(self, data_dir, dataset, config_dict, split, is_random_chirp=True, subset=None, noise_channel=False, testing_state=0):
        # parameters settings
        ...
        if subset is not None:
            if self.testing_state == 0:
                self.data_files = subset
            else:
    ...
    def __getitem__(self, index):
        testing_state = self.testing_state
            ...
            if radar_configs['data_type'] == 'RI' or radar_configs['data_type'] == 'AP':  # drop this format
                ...
                if isinstance(chirp_id, int):
                    ...
                    for idx, frameid in enumerate(
                            range(data_id, data_id + self.win_size * self.step, self.step)):
                        #if idx == 0:
                            #print('Creating enumerate for %d to %d' %(data_id ,(data_id+self.win_size)))
                        for cid, c in enumerate(chirp_id):
                        ...
                        if testing_state == 0:
                            ...
                            data_dict['image_paths']=None #this is only if testing. 
                else:
                    raise TypeError
            else:
